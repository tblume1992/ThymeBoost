...
if __name__ == '__main__':
    ...
    plt.plot(y)
    plt.show()
    boosted_model = ThymeBoost(verbose=0,
                               n_rounds=None,
                               normalize_seasonality=True,
                               )
    output = boosted_model.fit(y[:50],
                                  min_sample_pct=.2,
    ...
    predicted_output = boosted_model.predict(output, 50, trend_penalty=True)
    ...
    look = boosted_model.optimizer.cv_predictions
    plotting.plot_optimization(output, look, opt_type='cv', figsize=(12,8))


    look = boosted_model.changepoints
    boosted_model.scale_type
    updated_output = boosted_model.update(output, y[50:100])
    ...
    plt.plot(np.append(updated_output['yhat'], predicted_output['predictions']))
# %%
...
# %%
    ...
    output = boosted_model.optimize(y,
                                    trend_estimator=[['linear', 'moving_average']],
                                    # window_size=[4],
                                   ...
                                   optimization_strategy='cv')
    # output = boosted_model.fit(y.iloc[:-80].values,
    #                             trend_estimator=['linear', 'moving_average'],
    #                             init_trend='linear',
    #                             fit_type=['local', 'global'],
    #                             window_size=[12, 8, 4],
...
# %%
    ...
    df = pd.read_csv(r'Sales_and_Marketing.csv')
    df.index = pd.to_datetime(df['Time Period'])
    y = df['Sales']
    X = df['Marketing Expense'].to_frame()
