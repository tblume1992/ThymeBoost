...
if __name__ == '__main__':
    ...
    import pandas as pd
    import matplotlib.pyplot as plt
    import ThymeBoost as lp
    
   
    #Get bitcoin data
    quandl.ApiConfig.api_key = 'PReVnyjgXhGBMUez1KMA'
    ...
    plt.plot(forecast['multiplicative_terms'])
    future_df = model.make_future_dataframe(300)
    predicted = model.predict(future_df)
    plt.plot(predicted['yhat'])    
# %%
    boosted_model = ThymeBoost(seasonal_period = 365,
                         fit_type = 'local',
    ...
    output = boosted_model.fit(y, forecast_horizon = 30)
    boosted_model.plot_results()
    boosted_model.plot_components()
    plt.plot(np.append(output['yhat'], output['predicted']), color = 'red')
    plt.plot(predicted['yhat'], color = 'orange')
    plt.plot(y.values)
    plt.show()
    plt.plot(np.append(output['trend'].values, output['predicted_trend']))
    plt.show()
    plt.plot(np.append(output['seasonality'].values,output['predicted_seasonality']))
    plt.show()
    boosted_model.plot_components()
    boosted_model.plot_results()
    print(f"Trend %: {round(100*(output['predicted_trend'][-1] - output['predicted_trend'][0]) / output['predicted_trend'][0],2)}")
    plt.plot(forecast['yhat'])
    plt.plot(output['yhat'].values)
    print(np.mean(output['seasonality'].values))
    plt.show()
    plt.plot(forecast['trend'])
    plt.plot(output['trend'].values)
    plt.show()
    plt.plot(forecast['additive_terms'])
    plt.plot(output['seasonality'].values)
    plt.show()
